...
class DatabaseHelper(private val context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    companion object {
        ...
        const val TYPE_PROTECTED = 3
    }

    fun getProtectedSpeciesLogs(): List<HistoryItem> {
        val list = mutableListOf<HistoryItem>()
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_NAME WHERE $COLUMN_TYPE = $TYPE_PROTECTED ORDER BY $COLUMN_TIMESTAMP DESC", null)
        if (cursor.moveToFirst()) {
            do { list.add(extractItem(cursor)) } while (cursor.moveToNext())
        }
        cursor.close()
